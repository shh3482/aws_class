...
public class EX02_String {
	...
	public static void main(String[] args) {
		 ...
		 *   예시
		 *   String str = "abc"
		 *   str = str + "d"; // abc 뒤에 ㅇ를 이어 붙이는게 아니라 abcd라는 문자열을 새로 만들어 연결
		 * 
		 * StringBuffer(쓰레드 지원)와 StringBuilder 380페이지
		 * - char[]로 문자열을 관리
		 * - 둘의 차이는 쓰레드 지원 여부
		 * 
		 * String 클래스 메서드
		 * - indexOf(문자열) : 문자열이 있으면 시작 번지를 알려줌
		 * - equals(문자열) : 두 문자열이 같은지 비교하여 true/false 를 반환
		...
		System.out.println("str1dml 2번지부터 부분문자열은? " + str1.substring(2));
