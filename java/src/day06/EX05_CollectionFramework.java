...
public class EX05_CollectionFramework {
	...
	public static void main(String[] args) {
		 ...
		 *   - contains(값) : 값이 있으면 true, 없으면 false 를 반환
		 *   - size() : 현재 저장된 크기
		 ...
		 *   - LinkedList : 링크로 연결, 전체 탐색 느림, 증감 추가 삭제 빠름
		 ...
		 *   - V는 중복 허용
		 * - 기능
		 *   - get(k) : 키들 중에서 k 와 일치하는 객체를 반환
		 *   - put(k, v) : 키들 중에서 k 와 일치하는 객체가 없으면 k 와 v 를 추가. 있으면 v 만 수정
		 *   - remove(k) : 키들 중에서 k 와 일치하는 객체가 있으면 삭제 후 v 를 반환
		 *   - keySet() : 키값들을 Set 으로 만들어서 반환
		 *     - Map 에서 반복문을 이용할 때 활용
		 * 
		 * 컬렉션 프레임워크에서 객체를 추가, 삭제할 때 해당 객체가 있는지를 Objects.equals(o1, o2)를 이용하여 확인
		 * - Objects.equals(o1, o2)
		 *   - o1과 o2가 다른 클래스이면 false 를 반환
		 *   - o1과 o2중 하나라도 null 이면 false 를 반환
		 *   - o1.equals(o2)를 호출
		 *   => equals 를 오버라이딩해야 함
		 */
		...
		// remove 는 메서드 오버로딩에 의해 2개 있음. 하나는 객체, 하나는 정수 
		...
		// 값이 2인 객체 삭제. Integer 안붙여도 됨. set 은 번지를 이용한 메서드가 없음
		set.remove(2);
		System.out.println(set);
		...
		// List, Set 향상된 for 문 이용하기
		for ( String fruit : list3 ) {
			System.out.println(fruit);
		}
		
		// List, Set iterator 를 이용하기
		...
		// map 은 K와 V가 필요. 타입이 2개 필요
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("abc123", "123123");
		
		System.out.println(map);
		/* map 은 직접 반복문을 이용할 수 없음
