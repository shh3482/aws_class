...
public class HW10_StudentProgram {
	...
	public static void main(String[] args) {
		/*
		 * 학생의 성적을 관리하는 프로그램을 구현하세요. - 파일에 저장. 파일에서 불러오기 메뉴 1. 학생 등록 - 학년, 반, 번호, 이름을
		 * 입력받아 등록 - 학년, 반, 번호가 같은 학생은 등록 못함 2. 학생 삭제 - 학년, 반, 번호를 입력받아 삭제 3. 학생 조회 -
		 * 학년, 반, 번호를 입력받아 조회 4. 과목 등록 - 학년, 학기, 과목명을 입력받아 등록 - 같은 학년, 학기, 과목명을 가진 과목은
		 * 등록 못함 5. 과목 삭제 - 학년, 학기, 과목명을 입력받아 삭제 6. 과목 전체 조회 7. 학생 성적 추가 - 학생의 학년, 반,
		 * 번호를 입력받아 있으면 과목 학년, 학기, 과목명, 성적을 입력받아 추가 8. 학생 성적 삭제 - 학생의 학년, 반, 번호를 입력받아
		 * 있으면 과목 학년, 학기, 과목명을 입력받아 삭제 9. 프로그램 종료
		 */
		int menu = 0;
		final int EXIT = 9;
		// 학생 정보를 관리하는 리스트
	...
	private static void addSubjectScore(List<Student> students, List<Subject> subjects) {
		...
		SubjectScore subjectScore = new SubjectScore(0,0,"",score);
		...
		// 선택한 학생의 성적을 출력
		// 출력된 성적은 위에서 기입한 성적을 먼저 성적 List에 추가된걸 출력되게 변경
		//selectedStudent.printScore();
		
		// 학생정보에게 입력한 성적 정보를 주고 추가하려고 요청 후 성공 여부를 알려 달라고 함
	...
	private static void removeSubject(List<Subject> subjects) {
		...
		if (getSubjectBySubjects(subjects, subject) != null) {
			println("과목이 삭제 되었습니다.");
			subjects.remove(subject);
		}
		else {
			println("과목 정보가 올바르지 않거나 없는 과목입니다.");
			return;
		}
	...
	private static Subject getSubjectBySubjects(List<Subject> subjects, Subject subject) {
	...
	private static void removeStudent(List<Student> students) {
		...
		// 학생 정보를 이용하여 삭제하는데 학생 정보가 없으면 일치하는 학생이 없습니다. 출력후 종료
	...
	private static void searchStudent(List<Student> list) {
		...
		for (Student tmp : list) {
			...
			if (tmp.getGrade() == student.getGrade() && tmp.getClassNum() == student.getClassNum()
