...
public class EX01_Exception {
	...
	public static void main(String[] args) {
		 ...
		 *   - 예외 클래스 최고 조상 클래스
		 *   
		 * - RuntimeExecption 클래스
		 *   - 프로그램 실행 시 발생하는 예외
		 *   - 예외 처리를 하지 않아도 코드상 오류가 발생하지 않음
		 *   - NullPointerExecption : 객체가 만들어지지 않은 상황에서 객체에 접근
		 *   - ArithmeticExection : 정수/정수에서 0으로 나눌 때 발생
		 *   - ArrayIndexOutOfBoundsExecption : 배열이나 리스트의 잘못된 번지에 접근
		 *   
		 * - RuntimeExecption 클래스가 아닌 클래스
		 *   - 예외처리를 하지 않으면 코드상 오류가 발생 => 반드시 예외처리를 해야 함
		 *   
		 * 예외 처리 방법
		 * 1. 예외가 발생한 곳에서 직접 처리 523페이지
		 * - 예외처리하기
		 * - 문법
		 *   try{
		 *    예외가 발생할 수 있는 코드
		 ...
		 *    => try나 catch에 return이 있어서 메서드가 종료 되어도 종료되기 전에 무조건 실행
		 *  }
		 *  - 예외클래스명1은 예외클래스명2의 조상클래스가 오면 안됨
		 * 
		 * 2. 예외가 발생한 곳에서 처리하지 않고 호출한 메서드에게 넘기는 방법
		 * - 예외 처리 미루기, 예외 처리 던지기
		 * - 예외가 생길 코드에 예외 던지기
		 *   - 예외가 발생할 수 있는 상황을 조건문으로 처리하고 throw를 이용하여 예외를 던짐
		 *   
		 * - 메서드 선언부에 throws를 입력 후 발생할 수 있는 예외를 적어줌
		 *   - 단, RuntimeExecption과 자손 클래스들은 생략 가능
		 *   
		 * 예외 처리를 왜 해야하는가?
		...
		try{
			...
			System.err.println("런타임 예외가 발생했습니다.");
		}
		catch(Exception e) {
