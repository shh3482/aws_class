...
대리키
- 복합키 이거나 마땅히 기본키로 설정할 속성이 없을 때 사용하는 키
...
- 문자 
  ...
  - text : 긴 문자
  - blob : 바이너리 데이터
  - enum : 열거형. 지정된 값 중 하나. a,b,c중 하나만 넣음
...
- 실행 순서
    ...
    4. having 조건
    5. select *
      - MYSQL과 오라클에서 예외적으로 select에서 집계함수의 별명이 있는 경우 having보다 먼저 실행)
    6. order by
    7. limit
    
- 집계함수
  - group by 후 갯수 평균, 합등을 계한할 때 사용
...
# treuncate vs drop vs delete 처리를 확인하는 예제
