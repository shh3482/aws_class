...
# 가져온 이미지를 독립변수(행열배열)들과 종속변수(값배열)들로 반환
# 이미지들을 저장할때 폴더/숫자(라벨)/이미지파일 하는게 보통
...
def load_model_predict(image:str, width:int, height:int, file_name:str = 'mnist_model.pkl'):
  ...
  # 이걸 안하려면 reshape(1,-1)을 통해 1행짜리 2차원 리스트로 만들면 됨 위에 .reshape 제거후 => return model.predict([img])
...
if __name__ == '__main__':
  # download_mnist('day13/images')
  # images, labels = load_image_dataset('day13/images', 28, 28)
  # train_model_save_model(images, labels)
  print(load_model_predict('day13/6.png', 28,28))
  print(load_model_predict('day13/7.png', 28,28))
  print(load_model_predict('day13/66.png', 28,28))
